...
- Языки: **ru / kk / en** — распределены по **уникальным** сценариям (без дублей-переводов: каждая строка — своя формулировка, домен, сумма). Модальности: видео (149) + аудио (1).
...
- **Анализ содержания речи (запланировано):** встроить ASR (Whisper) + текстовые
  risk-сигналы (`financial_call_to_action`, домены, telegram, казино). Сейчас реальный
  детектор и веб-UI смотрят только на **медиа** (лицо/голос), а не на то, **что** говорится
  в видео. Содержание учитывается в датасете и в stub, но не в real-детекторе — это
  следующий шаг для целостного Демо 3.
...
**Речевые сценарии — УНИКАЛЬНЫЕ записи** (78 строк, все `combined_text` различны: своя формулировка + домен/сумма/telegram/промокод). Языки ru/kk/en распределены, но НЕ дублируют один смысл переводом.
